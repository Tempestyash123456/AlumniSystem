...
const MediaUploadZone: React.FC<{
...
}> = ({ current, currentType, onFile, pendingFile }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {previewUrl && (
                <button type="button" onClick={() => onFile(null)} style={{
...
const DocUploadZone: React.FC<{
    ...
    onFile: (f: File | null) => void;
    pendingFile?: File | null;
}> = ({ currentName, currentUrl, onFile, pendingFile }) => {
    const inputRef = useRef<HTMLInputElement>(null);
    const [drag, setDrag]   = useState(false);
    ...
    return (
        ...
        >
            ...
            <div style={{ height: 160, overflow: 'hidden', position: 'relative', background: 'var(--bg-hover)', flexShrink: 0 }}>
                ...
                <div style={{ position: 'absolute', top: 10, right: 10 }}>
                    <span style={{
                        ...
                        background: upcoming ? 'rgba(57,255,20,0.15)' : 'rgba(255,184,0,0.12)',
                        color: upcoming ? 'var(--neon-green)' : 'var(--neon-amber)',
                        border: `1px solid ${upcoming ? 'rgba(57,255,20,0.3)' : 'rgba(255,184,0,0.3)'}`,
                    }}>
            ...
            <div style={{ padding: '16px 18px', flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
                <h3 style={{ fontFamily: 'Orbitron, monospace', fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '0.04em', lineHeight: 1.4 }}>
                    {event.name}
                </h3>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ color: 'var(--neon-cyan)', fontSize: 12 }}>⏱</span>
                    ...
                    {/* Place */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ color: 'var(--neon-pink)', fontSize: 12 }}>📍</span>
                    ...
                    {event.documentName && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                            <span style={{ fontSize: 11 }}>{docIcon(event.documentName)}</span>
        ...
        <Modal open={!!event} title={event.name} onClose={onClose} width={720}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
                <span style={{
                    ...
                    background: upcoming ? 'rgba(57,255,20,0.12)' : 'rgba(255,184,0,0.1)',
                    color: upcoming ? 'var(--neon-green)' : 'var(--neon-amber)',
                    border: `1px solid ${upcoming ? 'rgba(57,255,20,0.3)' : 'rgba(255,184,0,0.3)'}`,
                }}>
                    {upcoming ? 'UPCOMING' : 'PAST'}
...
const EventEditorModal: React.FC<{
    ...
    onClose: () => void;
    onSave: (state: EditorState, mediaFile: File | null, docFile: File | null) => Promise<void>;
    saving: boolean;
    error: string;
    onClearError: () => void;
    ...
    return (
        ...
        >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {/* Document */}
                <DocUploadZone
                    currentName={editTarget?.documentName}
                    currentUrl={editTarget?.documentUrl}
                    pendingFile={docFile}
                    onFile={setDocFile}
                />
                ...
                <div style={{
                    ...
                    <Button
                        loading={saving}
                        onClick={() => onSave(form, mediaFile, docFile)}
                    >
...
export const EventsPage: React.FC = () => {
    ...
    const handleSave = async (
        ...
        let res;
        if (editTarget) {
            res = await eventsApi.update(editTarget.id, data, mediaFile, docFile);
        } else {
    ...
    const filtered = events.filter(ev => {
        ...
        const matchStatus =
            filterStatus === 'all' ||
            (filterStatus === 'upcoming' && isUpcoming(ev.startTime)) ||
            (filterStatus === 'past' && !isUpcoming(ev.startTime));
    ...
    return (
        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 24, height: '100%', minHeight: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 14, flexShrink: 0 }}>
                {[
                    { label: 'Total',    value: events.length, color: 'cyan'   },
